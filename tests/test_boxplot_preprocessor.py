...
def test_run_boxplot_uses_aggregated_y_columns_for_routed_dataframe(monkeypatch):
    ...
    assert captured["df"]["variable"].tolist() == ["metric", "metric"]
    assert captured["df"]["value"].tolist() == [15, 30]


def test_run_boxplot_use_raw_keeps_aggregation_semantics(monkeypatch):
    from indexly.visualization import boxplot_engine
    ...
    fake_render_static_boxplot.key = "clean"
    boxplot_engine.run_boxplot(SimpleNamespace(**args, use_raw=False), routed_df=df)
    fake_render_static_boxplot.key = "raw"
    boxplot_engine.run_boxplot(SimpleNamespace(**args, use_raw=True), routed_df=df)

    pd.testing.assert_frame_equal(rendered["raw"], rendered["clean"])


def test_run_boxplot_preserves_value_column_name_safely(monkeypatch):
    from indexly.visualization import boxplot_engine

    df = pd.DataFrame({"x": ["a", "a", "b"], "value": [10, 20, 30]})
    captured = {}
