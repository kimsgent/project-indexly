...
# --------------------------------------------------------------------------
# Safe Export Utility for XML Pipeline
# --------------------------------------------------------------------------
...
def safe_export(
    ...
    def _to_md_table(data: Any) -> str:
        ...
        if isinstance(data, dict):
            lines.append(f"<!-- Generated by Indexly {datetime.utcnow().isoformat()}Z -->\n")
    ...
    if export_format == "md":
        ...
        content = json.dumps(safe_results, indent=2, ensure_ascii=False) if isinstance(safe_results, (dict, list)) else str(safe_results)
