...
class LogReader:
    ...
    def find_logs_for_month(self, month: str) -> list[Path]:
        """Find NDJSON logs under all year folders for a month in MM format."""
        ...
        logs = [
            path for path in self.log_dir.glob(f"*/{month}/*.ndjson") if path.is_file()
        ]
    ...
    def build_unchanged_set_from_log(
        ...
        This is intentionally diagnostic for Phase 1. A past unchanged log entry
        is not a safe freshness signal by itself because the file may have been
        edited after the log was written.
        """
        try:
            records, _metadata = _parse_ndjson_records_from_path(log_path)
        except (OSError, ValueError) as exc:
            logger.warning("Could not read incremental index log %s: %s", log_path, exc)
            return set()
        ...
        for record in records:
            if record.get("event") != "FILE_INDEXED":
                continue
            if record.get("content_changed") is not False:
                continue
