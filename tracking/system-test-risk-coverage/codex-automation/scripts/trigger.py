...
PROCESS_REFERENCE = "tracking/system-test-risk-coverage/README.md"
DRAFT_STATUS = "draft_scaffold"
        ...
        if template_path.exists():
            ...
            if not out_path.exists():
                with open(template_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                content = prepare_draft_artifact_content(out_name, content)
                with open(out_path, 'w', encoding='utf-8') as f:
                    f.write(content)


def prepare_draft_artifact_content(out_name, content):
    """Mark generated worksheet artifacts as draft scaffolding."""
    if out_name.endswith(".md"):
        draft_notice = (
            "> Status: Draft scaffold generated by Codex automation.\n"
            "> This file is not an analysis result yet.\n"
            f"> Complete it according to `{PROCESS_REFERENCE}` after code inspection, "
            "risk assessment, remediation planning, and validation evidence are recorded.\n\n"
        )
        return draft_notice + content
    ...
    if out_name.endswith(".json"):
        ...
        data["artifact_status"] = DRAFT_STATUS
        data["analysis_completed"] = False
        data["process_reference"] = PROCESS_REFERENCE
        data["completion_requirements"] = [
            "Read and follow tracking/system-test-risk-coverage/README.md.",
            "Inspect relevant code and fault boundaries.",
            "Record risk assessment and validation plan.",
            "Record remediation results, test evidence, and residual risks.",
        ]
...
def write_analysis_prompt(run_folder, params):
    ...
    mode = params["mode"]
    process_guidance = (
        "Before filling or judging any generated worksheet artifact, read "
        f"`{PROCESS_REFERENCE}` and follow that tracking process.\n\n"
        "Treat `system-test-case-summary-worksheet.md` and "
        "`system-test-case-summary-worksheet.json` as draft scaffolding only. "
        "They are not evidence-backed analysis outputs until code inspection, "
        "risk assessment, remediation or validation planning, test evidence, "
        "and residual risks are recorded.\n\n"
    )
    if mode == "known_bug":
        prompt = (
...
def main():
    ...
    print(f"  2. Read and follow {PROCESS_REFERENCE}")
    print("  3. Use that prompt in Codex for analysis")
    print("  4. Update draft worksheet artifacts with findings and validation evidence")
