...
`metrics.json` is generated by `../scripts/regenerate_dashboard_metrics.py`. The worksheet JSON files remain the source of truth.
...
3. Run `python ../scripts/regenerate_dashboard_metrics.py` from this folder, or run `python tracking/system-test-risk-coverage/scripts/regenerate_dashboard_metrics.py` from the repository root.
4. Run `python ../scripts/regenerate_dashboard_metrics.py --check` before committing if you want a stale-metrics guard.
5. Open `index.html` locally to inspect the dashboard.
6. Commit the dashboard update with the worksheet changes.

## Baseline and Run Folder Rule

Do not edit the baseline seed documents in `../test-cases/2026-06-01-test-case-system-test-risk-coverage/` for run-specific outcomes. Create a new dated run folder from templates, then regenerate this dashboard. The script rejects worksheet JSON found outside `YYYY-MM-DD-test-case-<area-or-change>` folders.
