...
```bash
indexly migration-manager --dry-run
```

After confirming the actions:

```bash
indexly migration-manager
```
...
* [`search_fts5()` Logical Operators and Schema Compatibility](/features/_index.en.md#search)
* [`indexly watch`](config.md#watchdog-real-time-indexing)
* [`tag management`](tagging.md)

---

**Keywords:** database, migration, fts5 rebuild, schema versioning, sqlite, update-db, migrate-db, indexly
